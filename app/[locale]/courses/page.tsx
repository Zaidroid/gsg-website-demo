...
export default function CoursesPage() {
    ...
    // Translate the courses data from the static data file
    const translatedCourses = courses.map(course => ({
        ...course,
        title: t(`data.${course.id}.title`),
        description: t(`data.${course.id}.description`),
        translatedCategory: t(`categories.${course.category}`),
        translatedStatus: t(`status.${course.status}`),
        translatedLevel: t(`levels.${course.level}`)
    }));
    ...
    return (
        <div className="bg-transparent min-h-screen">
            ...
            <div className="mx-auto max-w-7xl px-6 lg:px-8 py-10 sm:py-16">
                {/* Filters & Search */}
                <div className="flex flex-col items-center justify-between gap-4 md:flex-row relative z-10">
                ...
                {/* Grid */}
                <div className="mt-10 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-10 border-t border-gray-100 dark:border-white/5 lg:mx-0 lg:max-w-none lg:grid-cols-3">
                                    ...
                                    <div className="relative z-10">
                                        <div className="flex items-center justify-between mb-4">
                                            <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ring-1 ring-inset ${course.status === 'Open' ? 'bg-green-50 text-green-700 ring-green-600/20 dark:bg-green-900/20 dark:text-green-400' :
                                                ...
                                                {course.translatedStatus}
                                            </span>
                                            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{course.translatedCategory}</span>
                                        </div>
                            ...
                            >
                                <div className="flex flex-wrap gap-4 mb-6">
                                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 text-sm font-bold">
                                        <Calendar size={16} /> {t("labels.started")}: {selectedCourse.startDate}
                                    </div>
                                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300 text-sm font-bold">
                                        <Clock size={16} /> {t("labels.duration")}: {selectedCourse.duration}
                                    </div>
                                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300 text-sm font-bold">
                                        <BarChart size={16} /> {t("labels.level")}: {selectedCourse.translatedLevel}
                                    </div>
                                </div>
